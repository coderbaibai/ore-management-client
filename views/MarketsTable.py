...
from qfluentwidgets import (BodyLabel, InfoBar, setTheme, CaptionLabel, InfoBarPosition,
...
class FileItem(BodyLabel):
    ...
    def __init__(self,type:int,name:str,size:str,bucket:str,parent=None,highlight_start=-1,highlight_end=-1,path :str=""):
        ...
        self.__bucket_widget = CaptionLabel(bucket,self)
        self.__bucket_widget.setFixedWidth(100)
        ...
        self.__layout.addWidget(self.__bucket_widget,Qt.AlignmentFlag.AlignRight)
...
class TableHeader(CaptionLabel):
    ...
    def __init__(self,parent=None):
        ...
        self.__size.setFixedWidth(130)
        self.__type_widget = CaptionLabel('所在桶',self)
        ...
        self.__layout.addWidget(self.__size)
        self.__layout.addWidget(self.__type_widget,Qt.AlignmentFlag.AlignRight)
...
class FilesTable(QWidget):
    ...
    def __init__(self,parent=None):
        ...
        self.menu.addAction(Action(FIF.CUT, '删除', triggered=self.handle_item_delete_signal))
    ...
    def update(self,currentPath:list[str]):
        ...
        if len(currentPath)==1:
            ...
            self.manager.finished.connect(self.handle_market_update_response)
            self.manager.get(request)
        elif len(currentPath)==2:
            url = QUrl(gConfig['server']['spring']['url']+'/market/items/current')
            query = QUrlQuery()
            query.addQueryItem("id", self.itemMapper[currentPath[1]])
            url.setQuery(query)
            request = QNetworkRequest(url)
            self.manager.finished.connect(self.handle_item_update_response)
            self.manager.get(request)
        else:
            return
    ...
    def handle_market_update_response(self,reply:QNetworkReply):
        ...
        if res['code'] == 0:
            ...
            for dic in res['data']['markets']:
                name = dic['name']
                type = FileType.directory
                size = '-'
                bucket = '-'
                self.itemMapper[dic['name']] = str(dic['id'])
                self.items.append(FileItem(type,name,size,bucket,self))
            while self.fileLayout.count() > 1:
                item = self.fileLayout.takeAt(1)  # 取出布局中的第一个子项
                widget = item.widget()  # 获取子项对应的组件
                if widget is not None:
    ...
    def handle_item_update_response(self,reply:QNetworkReply):
        self.manager.finished.disconnect()
        res = json.loads(reply.readAll().data().decode())
        if 'code' not in res:
            print(f"❗ Error Code: {res['status']}\n")
        ...
        for dic in res['data']['marketItems']:
            ...
            type = FileType.file
            size = dic['size']
            bucket = dic['bucketName']
            self.items.append(FileItem(type,name,UnitTranslator.convert_bytes(size),bucket,self,path=dic['path']))
        while self.fileLayout.count() > 1:
            item = self.fileLayout.takeAt(1)  # 取出布局中的第一个子项
            widget = item.widget()  # 获取子项对应的组件
            if widget is not None:
        ...
        for i in self.items:
            ...
            i.show()
        self.number_signal.emit(len(self.items))
        self.show_signal.emit(False)
        reply.deleteLater()

    def handle_item_delete_signal(self):
        names = self.getTargets()
        ids = []
        for i in names:
            if i in self.itemMapper.keys():
                ids.append(self.itemMapper[i])
        if len(ids) == 0:
            return
        data_to_post = {}
        data_to_post['markets'] = []
        
        for id in ids:
            data_to_post['markets'].append({"id": int(id)})
            json_data = json.dumps(data_to_post).encode('utf-8')
            url = QUrl(gConfig['server']['spring']['url']+'/market/delete')
            request = QNetworkRequest(url)
            request.setHeader(QNetworkRequest.KnownHeaders.ContentTypeHeader, "application/json")
            reply = self.manager.post(request, json_data)
            reply.finished.connect(lambda: self.on_item_delete_response(reply=reply))

    def on_item_delete_response(self,reply:QNetworkReply):
        res = json.loads(reply.readAll().data().decode())
        if 'code' not in res:
            print(f"❗ Error Code: {res['status']}\n")
            print(f"❗ Error msg: {res['error']}")

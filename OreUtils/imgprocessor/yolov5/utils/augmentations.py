...
def letterbox(im, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    # print("augmentation.py 138: np.max(im):",np.max(im),np.min(im))
    top, bottom = int(round(dh - 0.1)), int(round(dh + 0.1))
    left, right = int(round(dw - 0.1)), int(round(dw + 0.1))
    im = cv2.copyMakeBorder(im, top, bottom, left, right, cv2.BORDER_CONSTANT, value=color)  # add border
    # print("augmentation.py 142: np.max(im):",np.max(im),np.min(im))
...
def random_perspective(im,
    ...
    # print(M[:2],width,height)

    def wrap(image,rot_mat,dst_h,dst_w):
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            im = cv2.warpAffine(im.astype(np.float32), M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            # im = wrap(im,M[:2],width,height)
    ...
    if n:
        ...
        targets = targets[i]
        targets[:, 1:5] = new[i]
        # print("augmentations 259:",np.max(im),np.min(im))

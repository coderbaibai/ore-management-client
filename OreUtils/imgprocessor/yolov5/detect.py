...
from utils.plots_16 import Annotator, colors, save_one_box
...
def run(
        ...
        label_id = 0,
        imgsz=(320, 1472),  # inference size (height, width)
    ...
    mutou_num = 0
    length_distribution = {
        "mei": {"[0,40]": 0, "[40,150]": 0, "[150,200]": 0, "[200,300]": 0, "[>300]": 0},
        "shi": {"[0,40]": 0, "[40,150]": 0, "[150,200]": 0, "[200,300]": 0, "[>300]": 0},
        "mutou": {"[0,40]": 0, "[40,150]": 0, "[150,200]": 0, "[200,300]": 0, "[>300]": 0}
    }
    source = str(source)
...
def parse_opt(label_class, imgsz, weight,source,project, conf_thres, iou_thres, save_txt = False):
    parser = argparse.ArgumentParser()
    parser.add_argument('--label-id',type=int,default=label_class,help='the label id to be annotated')
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/test1.yaml', help='(optional) dataset.yaml path')
    ...
    parser.add_argument('--save-txt', default = save_txt, action='store_true', help='save results to *.txt')
...
def main(label_class = 0, imgsz = [640, 640], weight_path = 'yolov5/weights/best.pt', data_path = None, target_path= './data/modelAnchorTest', conf_thres = 0.4, iou_thres = 0.3):
    
    labels_path = ''
    
    opt = parse_opt(label_class, imgsz, weight_path, 
                    data_path, target_path , conf_thres, iou_thres,
                    True)
...
if __name__ == "__main__":
    ...
    data_path = "./data/dingwei_shi"
    target_path = "./data/modelAnchorTest"            
    conf_thres, iou_thres = 0.4, 0.3
    main(label_class, imgsz, weight_path, data_path, target_path, conf_thres, iou_thres)

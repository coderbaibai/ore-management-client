...
from qfluentwidgets import (InfoBar, InfoBarPosition, TableWidget, ComboBox, FluentWindow,
                            MessageBoxBase, LineEdit, SubtitleLabel, setFont,FluentLabelBase)
from qfluentwidgets import FluentIcon as FIF

from models.MarketsWidgetHeader import MarketsWidgetHeader
from models.MarketsTable import FilesTable
...
class MarketWidget(SubtitleLabel):
    ...
    def __init__(self,text:str,parent=None):
        ...
        self.header = MarketsWidgetHeader(self)
        ...
        self.header.new_market_signal.connect(self.handle_new_market_signal)
    ...
    def showEvent(self, event):
        ...
        self.table.update(['全部数据市场'])
        self.header.pushPath(['全部数据市场'])
        self.header.updateBreadBar()
        # 你可以在这里添加其他需要在显示时执行的操作
    
    def handle_new_market_signal(self):
        w = NewMarketDialog(self.window())
        if w.exec_():
            data_to_post = {}
            data_to_post['name'] = w.name_input.text()
            # 转换为JSON字符串并编码为字节
            json_data = json.dumps(data_to_post).encode('utf-8')

            url = QUrl(gConfig['server']['spring']['url']+'/market/add')
            request = QNetworkRequest(url)
            request.setHeader(QNetworkRequest.KnownHeaders.ContentTypeHeader, "application/json")

            # 发送 POST 请求
            self.manager.finished.connect(self.handle_add_market_signal)
            self.manager.post(request, json_data)

    def handle_add_market_signal(self,reply:QNetworkReply):
        self.manager.finished.disconnect()
        res = json.loads(reply.readAll().data().decode())
        if 'code' not in res:
            print(f"❗ Error Code: {res['status']}\n")

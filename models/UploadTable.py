...
class UploadTable(SubtitleLabel):
    ...
    def __init__(self,parent=None):
        ...
        self.uploader_cnt = 1000/gConfig['client']['update-time']
    ...
    def update(self):
        ...
        for tmp in temps:
            self.items.append(UploadItem(tmp.id,tmp.type,tmp.name,str(tmp.size),tmp.bucket,tmp.cloud,tmp.local,self))
    ...
    def start_upload(self,bucket,path,local):
        self.update()
            ...
            if uploader.is_finished():
                ...
                for tmp in self.items:
                    if tmp.getId()==id:
                        ...
                        deltaSize = uploader.get_delta()
                        if deltaSize==0:
                            self.uploader_cnt  = self.uploader_cnt+1000/gConfig['client']['update-time']
                        else:
                            tmp.setRate(UnitTranslator.convert_bytes(math.ceil(deltaSize*self.uploader_cnt))+'/s')
                            self.uploader_cnt = 1000/gConfig['client']['update-time']
